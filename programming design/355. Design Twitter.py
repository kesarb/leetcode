...
class Twitter:
    ...
    def __init__(self):
        self.following = {}#userid=set(), follow, unfollow
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        if userId in self.posts:
            twitts.extend(self.posts[userId][:10])
        if userId in self.following:
            for i in self.following[userId]:
                if i in self.posts:
                    twitts.extend(self.posts[i][:10])

        twitts.sort(key=lambda x: x[0], reverse=True)
        
        return [j for i,j in twitts[:10]]
    ...
    def unfollow(self, followerId: int, followeeId: int) -> None:
        if followerId in self.following:
            ...
            if  self.following[followerId] == 0:
