...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def dfs(i,j, visit, s, node):
            if not node or (i,j) in visit:
                return ""
            ...
            for u, v in neighbours:
                ...
                if isSafe(u1,v1) and (u1,v1) not in visit:
                    c = board[u1][v1]
                    if c in node.childrens:
                        dfs(u1,v1, visit, s+c, node.childrens[c])
        ...
        for i in range(m):
            for j in range(n):
                if board[i][j] in root.childrens:
                    dfs(i,j, visit, board[i][j], root.childrens[board[i][j]])
